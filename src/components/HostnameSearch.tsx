...
interface HostnameSearchProps {
  ...
  onManualSearch?: (hostname: string) => void;
  isSearching?: boolean;
}

const HostnameSearch = forwardRef<HTMLInputElement, HostnameSearchProps>(({ hostnames, onHostnameSelect, onManualSearch, isSearching = false }, ref) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (showResults && filteredHostnames.length > 0) {
      switch (e.key) {
        case 'ArrowDown':
          e.preventDefault();
        ...
        case 'Enter':
          ...
          } else if (onManualSearch && searchTerm.trim()) {
            handleManualSearch();
          }
          break;
        case 'Escape':
    ...
    } else if (e.key === 'Enter' && onManualSearch && searchTerm.trim()) {
      e.preventDefault();
      handleManualSearch();
    }
  };
      ...
      <div className="flex space-x-2">
        ...
        {onManualSearch && (
          <button
            ...
            className="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg transition-colors text-sm font-medium flex items-center space-x-2"
          >
            {isSearching ? (
              <>
